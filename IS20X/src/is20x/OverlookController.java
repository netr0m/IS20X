...
public class OverlookController implements Initializable, ControlledScreen {
    ...
    String currentUser = loginCtrl.currentUser;
    boolean teachermode = loginCtrl.teachermode;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        if (teachermode == true) {
            ...
            try {
                ...
                data = FXCollections.observableArrayList();
                // Execute query and store result in a resultset
                ResultSet rs = conn.createStatement().executeQuery("SELECT u.name, a.mod1, a.mod2, a.mod3, a.mod4, a.mod5, a.mod6, a.mod7, a.mod8, a.mod9, a.mod10, a.mod11, a.mod12, a.mod13, a.mod14 FROM approvals a INNER JOIN user u ON a.studentID = u.userID WHERE u.username = '" + currentUser + "';");
                while (rs.next()) {
                    //get string from db,whichever way 
                    data.add(new User(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8), rs.getString(9), rs.getString(10), rs.getString(11), rs.getString(12), rs.getString(13), rs.getString(14), rs.getString(15)));
                }
